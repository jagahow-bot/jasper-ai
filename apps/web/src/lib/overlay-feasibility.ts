...
const SECOND_LAYER_RE =
  /第二[層层]|二[層层]|兩[層层]|两层|各自|獨立子|独立子|子配置|sleeve|50\s*[%％].{0,40}50\s*[%％]|(?:AI|科技|人工).{0,48}(?:避險|避险|hedge)|(?:避險|避险|hedge).{0,48}(?:AI|科技|人工)/i;
...
export function detectSecondLayerAiHedgeIntent(
  ...
  const textPair =
    AI_THEME_RE.test(corpus) &&
    HEDGE_THEME_RE.test(corpus) &&
    (SECOND_LAYER_RE.test(corpus) || themePair);

  const detected =
    aiTickers.length > 0 &&
    hedgeTickers.length > 0 &&
    (themePair || textPair || SECOND_LAYER_RE.test(corpus));
...
function inferAiHedgeSleevePlan(
  ...
  // Substantial dual-sleeve without explicit % → treat as 50/50 for feasibility.
  if (
