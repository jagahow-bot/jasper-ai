...
| **對話式 Overlay（Conversational Overlay）** | 多輪對話捕捉客戶需求 → 結構化 Overlay JSON → 映射 `BacktestRequest` → 基準 vs 調整後雙軌回測 | 從「填表設參數」升級為「與客戶同頻的量化對話」，RM 簽核後方可執行 |
...
### 3.4 對話式 Overlay（Conversational Overlay）

針對 Julius Baer 挑戰中「從 onboarding 到 portfolio advisory 的無縫個人化旅程」，JASPER 新增 **Conversational Overlay** 流程：RM 以自然語言記錄客戶需求（流動性事件、產業集中度、風險趨避等），AI 在多輪對話中萃取並結構化為可審計的 Overlay JSON，經 RM 確認簽核後映射至 `BacktestRequest`，與機構基準組合並行回測，產出「調整前 vs 調整後」的機構級比較報告。AI 僅負責意圖萃取與追問澄清，不產生自由下單指令；所有績效數字仍由量化引擎計算並經 `narrative_facts` 校驗。完整流程、Schema 與 PoC 實作順序見 [Conversational Overlay 流程設計](./Conversational-Overlay-Flow.md)。
